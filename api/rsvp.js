  ...
  try {
    ...
    if (req.method === "DELETE") {
      try {
        ...
        if (!kv) {
          return res
            .status(501)
            .json({ ok: false, error: "Delete requires KV configured" });
        }
        ...
        return res
          .status(200)
          .json({ ok: true, deleted: list.length - filtered.length });
      } catch (e) {
    ...
    if (user && pass) {
      try {
        ...
        const subject = entry.attending
          ? `✅ ${entry.name} sẽ tham dự lễ tốt nghiệp`
          : `❌ ${entry.name} không thể tham dự`;
        const html = `
          <h2>${subject}</h2>
        ...
        console.warn(
          "Email send skipped or failed (nodemailer missing or unusable):",
          e?.message || e,
        );
      }
    }
    ...
    return res.status(200).json({
      ok: true,
      message: "RSVP processed (email sent if configured)",
      id: entry.id,
      persisted,
      storage,
    });
