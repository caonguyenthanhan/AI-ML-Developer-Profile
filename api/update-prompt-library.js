...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (promptsDataStart === -1 || promptsDataEnd === -1) {
      return res
        .status(500)
        .json({ error: "Could not find promptsData in HTML file" });
    }

    // Extract current promptsData
    const promptsDataSection = htmlContent.substring(
      promptsDataStart,
      promptsDataEnd,
    );
    const promptsDataMatch = promptsDataSection.match(
      /const promptsData = (\[[\s\S]*?\]);/,
    );
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Could not parse existing prompts data" });
    }
    ...
    const newHtmlContent =
      htmlContent.substring(0, promptsDataStart) +
